	...
	for app in modules_to_check:
		...
		try:
			...
			if hasattr(app_render_module, "renderers"):
				...
				print "Skipping app '%s'... ('%s.renderers' does not contain 'renderers' var (list of render classes)" % (app, app)
		except AttributeError:
			print "Skipping app '%s'... (Error importing '%s.renderers')" % (app, app)
			continue
